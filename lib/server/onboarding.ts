...
/** business_profiles column patch for a set of facts. Field paths map 1:1 to
 * columns; unknown fields never reach the database. */
export function profilePatch(
  facts: { fieldPath: OnboardingField; value: string | string[] }[],
) {
  return Object.fromEntries(
    facts.map((fact) => [fact.fieldPath, factValueForProfile(fact.value)]),
  ) as Partial<Record<OnboardingField, string | string[]>>;
}
