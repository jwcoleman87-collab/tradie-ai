  ...
  return (
    <>
      <span className="title-main">
        {separator < 0 ? title : title.slice(0, separator)}
      </span>
      {separator >= 0 && (
        <span className="title-sub">{title.slice(separator + 3)}</span>
      )}
    </>
  );
}
...
function businessFacts(snapshot: Snapshot) {
  ...
  return {
    greenVac,
    location:
      snapshot.businessProfile?.base_location ||
      (greenVac ? 'Canberra & Southern NSW' : ''),
    services:
      snapshot.businessProfile?.services.join(' · ') ||
      (greenVac ? 'Compact hydro excavation · locate & expose' : ''),
  };
}
...
export function BusinessBanner({
  ...
  return (
    <section
      className={`business-band ${facts.greenVac ? 'business-band-greenvac' : ''}`}
      aria-label="Your business"
    >
      {facts.greenVac && (
        <Image
          className="business-banner-photo"
          src="/brands/greenvac-banner.jpg"
          alt=""
          fill
          sizes="100vw"
          unoptimized
      ...
      <div className="business-band-content">
        <div className="business-band-identity">
          <div className="business-logo-badge">
            <BrandMark
              brand={findWorkspaceBrand(snapshot.workspace.name)}
              alt={snapshot.workspace.name}
              initials={snapshot.workspace.name.slice(0, 2).toUpperCase()}
            />
          </div>
          <div className="business-band-copy">
            <span className="business-trade">
              {facts.greenVac
            ...
            <p>
              {facts.location}
              {facts.greenVac && <>{facts.location && ' · '}0408 362 590</>}
              {!facts.location &&
                !facts.greenVac &&
        ...
        <div className="business-connections">
          <span>{connected.length ? 'Connected' : 'Your connections'}</span>
          <div>
            {connected.length ? (
    ...
    <footer className="crew-footer">
      {snapshot && facts && (
        <details className="business-details">
          <summary>
            Business details <ChevronDown size={13} />
          </summary>
          <div>
            {facts.location && (
              <p>
            ...
            {facts.greenVac && (
              <>
                <p>
                  <Zap size={13} />
                  <span>Electrical · NBN · Gas · Water · Stormwater</span>
                </p>
                <p>
                  <Phone size={13} />
                  <a href="tel:0408362590">0408 362 590</a>
                </p>
                <p>
                  <Globe size={13} />
                  <a
                    href="https://greenvac.com.au"
                    target="_blank"
                    rel="noreferrer"
                  >
                    greenvac.com.au
            ...
            {!facts.location && !facts.services && (
              <p>Business information appears here after setup.</p>
            )}
          </div>
        </details>
      )}
      <div className="crew-privacy">
        <ShieldCheck size={16} />
        <div>
          <strong>Private by default</strong>
          <p>Nothing goes out without your say-so.</p>
        </div>
      </div>
    </footer>
  );
...
export function MetadataDisclosure({
  ...
  useEffect(() => {
    try {
      ...
      /* Session state works without storage. */
    }
  }, [key]);
  return (
    <details
      className={`metadata-disclosure ${className}`}
      open={open}
      onToggle={(event) => {
    ...
    >
      <summary>
        {summary}
        <ChevronDown className="disclosure-chevron" size={14} />
      </summary>
...
export function BriefDisclosure({ children }: { children: ReactNode }) {
  const [open, setOpen] = useState(false);
  return (
    <details
      className="proposal-full-brief"
      open={open}
      onToggle={(event) => setOpen(event.currentTarget.open)}
    >
      <summary>
        {open ? 'Hide full brief' : 'Show full brief'} <ChevronDown size={13} />
      </summary>
      <div>{children}</div>
    </details>
  );
